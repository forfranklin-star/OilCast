...
def test_missing_factor_is_never_zero_filled():
    ...
    # 去掉 brent（无法构造 Brent-WTI 价差）、且无 GPR/新闻事件时，地缘组仍由内置十年重大
    # 事件年表展开的 hist_geo_regime / hist_china_supply_regime 支撑——它们是随仓库、可审计的
    # 真实数据，故地缘因素仍可用，不因外部新闻/价差源暂时缺失而被误判为"无地缘信息"。
    ...
    assert avail2["geopolitical_risk"] is True
    assert feats2["gpr_level_z"].isna().all() and feats2["geo_premium_chg5"].isna().all()
    # 仅当连分主题历史年表状态锚也缺失（地缘信息彻底为空）时，地缘因素才整体判为不可用
    feats2b = feats2.copy()
    for _c in ("hist_geo_regime", "hist_china_supply_regime"):
        feats2b[_c] = np.nan
    assert factor_availability(feats2b)["geopolitical_risk"] is False
