...
程序可完全在你自己的电脑（Windows / macOS / Linux）上运行，数据、模型、报告都保存在
本地，断网/换电脑可用学习快照导入导出（见第 5 节）。

### 3.1 一键脚本（推荐）

前置：安装 **Python 3.10+**（Windows 安装时勾选 *Add Python to PATH*）。解压/克隆仓库后：

- **Windows**：双击或在 cmd/PowerShell 运行
  ```bat
  scripts\run_local.bat
  :: 首次就带代理： scripts\run_local.bat http://127.0.0.1:7890
  ```
- **macOS / Linux**：终端运行
  ```bash
  bash scripts/run_local.sh
  # 首次就带代理： bash scripts/run_local.sh http://127.0.0.1:7890
  ```

脚本会自动：创建 `.venv` 虚拟环境 → 安装依赖 → 代理连通性自检 → 采集真实数据、训练并
生成报告 → 启动本地控制台。完成后浏览器打开 **http://localhost:8501** 即可（`Ctrl+C` 退出）。
...
python -m oilcast.pipeline.main          # 采集→训练→生成报告（只用真实数据，缺失会明示）
...
streamlit run src/oilcast/app.py         # 交互式控制台（侧栏可导出/导入学习快照）
```

产物：`reports/latest/index.html`（浏览器直接打开）、`latest.json`、
`reports/archive/YYYY-MM-DD.{json,html}`、`data/oilcast.db`。

### 3.3 代理服务器（本地运行、海外信源不可达时）

海外行情/宏观源（CNBC、FRED、ECB 等）在部分网络下需要代理。代理只在**本地命令行/脚本**
侧配置，支持 HTTP 与 SOCKS5（SOCKS5 自动用 `socks5h://` 远端解析 DNS）。设置方式
（优先级从高到低）：

1. **命令行参数（推荐）**：
   ```bash
   python -m oilcast.pipeline.main --proxy http://127.0.0.1:7890   # 设置并持久化
   python -m oilcast.pipeline.main --proxy socks5://127.0.0.1:1080
   python -m oilcast.pipeline.main --proxy-test                    # 只做连通性自检后退出
   python -m oilcast.pipeline.main --no-proxy                      # 清除代理、改回直连
   ```
   代理会持久化到本机 `data/proxy.json`，之后无论命令行还是一键脚本都自动继承。
2. **一键脚本直接带代理**：`bash scripts/run_local.sh http://127.0.0.1:7890`
   （Windows：`scripts\run_local.bat http://127.0.0.1:7890`）。
3. **环境变量**：`export OILCAST_PROXY=socks5://127.0.0.1:1080`（Windows cmd 用
   `set OILCAST_PROXY=...`）。
4. **配置文件** `config.yaml → system.network.proxy_url`（默认空=直连）。

**信源不可达时的暂停行为**：在本地终端直接运行（或用一键脚本）时，若某主机在网络层
连续不可达（连接超时/被拒，区别于 403/429 限频），程序会暂停并**倒计时 60 秒**，提示你：
① 在代理软件里切换节点后直接回车重试；② 直接粘贴新代理地址回车；③ 不操作则倒计时
结束自动跳过该信源、继续后续步骤（拿不到的数据如实标记 unavailable，绝不补齐）。
每个主机每次运行最多暂停一次。Streamlit 网页控制台与 GitHub Actions 等无交互终端环境
**不会暂停、不会卡死**（网页端不提供代理设置，按你启动它时本机已保存的代理/环境运行，
不可达信源自动跳过）。倒计时秒数与开关见 `config.yaml → system.network`
（`proxy_pause_enabled: auto/on/off`、`proxy_pause_seconds`）。

本地为**手动运行**：需要更新时执行一次 `scripts/run_local`（或 `python -m
oilcast.pipeline.main`）即可，不包含任何常驻唤醒或定时自动任务；每日自动更新只由云端
GitHub Actions（见第 6 节）负责，与本地运行相互独立、互不影响。
...
| 上海原油 INE SC | **新浪国内期货全月份合约（`SC`+YYMM，2018-03 上市至今）按每日持仓量最大重建主力、同合约换月比例复权的连续日K（人民币/桶）**；新浪 `SC0`/东方财富 `113.sc0` 仅作失败回退。原始 `SC0` 直接拼接不同月份合约，会在换月日（尤其近月交割前逼仓）制造虚假跳空，故不复用其拼接；以真实 USDCNY 换算美元后再与 Brent 算升贴水 | 否 |
...
- **上海原油**以人民币/桶报价，与美元报价品种**不直接做水平价差**；
...
- **主力连续换月校正（`data_sources/sc_contracts.py`）**：新浪 `SC0` 把不同到期月份合约直接首尾拼接，
  **换月日会产生跨合约虚假跳空**——实证 2026-09-15 主力由 SC2610 切到 SC2611，`SC0` 用近月 SC2610
  交割前逼仓高点 904.7 元接新主力 838.6 元，制造 −7.3% 的假跌（同期 SC2611 自身仅小涨）。系统改为
  枚举全部月份合约（`SC`+YYMM，2018-03 上市起），每日按**持仓量最大**确定主力（持仓并列按代码确定性
  tie-break），换月日用新合约自身前收盘计算同合约收益，并做**比例复权**（最新日复权因子=1，向过去在
  换月边界累乘），输出连续的 `adj_close/adj_ret`；日收益与同合约收益数值一致（误差 1e-16）。全历史
  102 个合约、92 次换月，消除单日 |涨跌|>4% 的假跳空 17 个。月份合约 K 线缓存于
  `data/cache/sc_contracts/SC{YYMM}.csv`（随包发布，首次运行免回填，每日只强刷最近 10 个合约）。
  重建失败时回退原始 `SC0` 并在谱系表如实标注"未换月调整"，绝不静默。
...
- **海外四品种十年全程同一交易日口径、不被分时窗口改写**：WTI 与美燃油同属 **NYMEX**、Brent 与伦敦柴油
  同属 **ICE**，组内两品种共享同一官方结算时刻（严格同刻）；NYMEX 与 ICE 的结算时刻相差约 1 小时，但都落在
  **同一 UTC 交易日**，CNBC 日 K 时间戳统一 `normalize()` 到该交易日（实证 2026-09-18：WTI 99.60、
  Brent 103.17、HO 5.04、Gasoil 1485.25）。十年日频按交易日对齐，跨市场外生特征再按各市场真实收盘时刻做
  as-of（只取已落定行情，无未来函数）。**这四个品种的价格序列十年始终用交易所日 K 结算价，不被任何分时
  窗口覆盖**，保证口径一致、模型不被亚洲品种的对齐窗口扰动。
- **上海原油独立对齐、独立学习（自 2026-06-03 起），不影响其他品种**：上海 INE 是唯一的亚洲时段品种，
  在分时覆盖窗口内**只把上海一个品种**的日价锚到其真正收盘——北京 02:30 夜盘收盘（此时欧美电子盘仍在交易，
  用户已确认按盘中价、不要求开/收盘价）；更早日期维持 4.4 的日 K（持仓量主力复权）口径。上海模型需要的
  **同时刻外盘价**（用于算对布升贴水、跨市场相对强弱）在 `build_features(target="shanghai_crude")` 内部从
  02:30 面板取 brent/wti，**仅作上海的外生特征、不回写全局 prices**；海外品种各自建模时（target=wti 等）
  完全不启用该融合（单测锁定 WTI 同步日数=0）。因此上海的对齐与学习是**隔离的**，绝不改变其他四品种结果。
  上海 02:30/15:00 面板同样按 4.4 的换月复权因子校正（`roll_adjust_panel_sc`），使其价格序列、跨市场外生
  特征、美元升贴水三处口径一致。
- **同刻对比仅供展示、双口径标注不混用**：报告"交易时段·同一真实时刻"面板仍把**五个品种的 02:30 同刻价**
  并列（02:30 主截面 + 15:00 对照截面 + 升贴水时序 + 分时根数/来源），供严格同时刻横向比较；但这只是展示层
  与上海建模输入，海外品种自身的价格序列/预测仍基于交易所日 K。**免费分时源历史有限**（新浪 15 分钟 K 仅
  回溯约百余交易日、CNBC `1H` 约 3 个多月，实测同刻面板自 2026-06-03 起），十年段无法逐时刻对齐，这是数据
  可得性的硬边界，报告与代码均如实标注、不宣称十年全程同刻；面板随每日增量积累，窗口会越来越长。
- **无泄漏 A/B 裁决**：上海分时校准在覆盖窗口滚动样本外回测，MAE 不劣于日频方予保留，变差即回退
  （`build_features(intraday_session=...)` 可一键关闭）。
...
- **地缘时变因子（刻画"边际升级"，非常年常数）**：
  ...
  - **重大事件 regime 分级（`data/reference/major_events.yaml` 的 `phases`）**：一场长期冲突
    ...
    公开可核验日期与来源。
  - **十年分主题重大事件年表（与价格样本区间匹配，2016-09 至今 35 起、每条带来源 URL）**：
    RSS 新闻只回溯近 30 天，若十年训练窗内没有事件历史，模型就学不到稳定的地缘/供给/需求敏感度
    （此前上海 INE 地缘权重被压到约 6.8% 的根因）。年表按主题逐日展开为 4 条日频状态特征：
    `hist_geo_regime`（战争/制裁/袭船/设施遇袭）、`hist_supply_regime`（OPEC+产量政策、SPR、
    寒潮/运河等实物中断）、`hist_demand_regime`（疫情等需求冲击）、`hist_china_supply_regime`
    （**专门标记直接扰动对华中东供油链路**的事件：伊朗制裁与豁免、海湾油轮战、Abqaiq 遇袭、
    2024/2025/2026 伊朗-以色列战事、霍尔木兹封锁）。分主题挂载使各品种在十年多起同类事件上
    学到真实敏感度差异——真实库复算中上海 INE 的 `hist_china_supply_regime` 细特征重要性
    （约 0.058）为五品种最高（WTI 约 0.030、Brent 约 0.027），与其"可交割中东含硫油、中国是
    伊朗原油主要买家"的经济定位一致；年表缺失时这些列整列 NaN（显式缺失，绝不造数）。
  - **新重大事件自动扩充（人工年表 + 机器核验双轨，`engineering._detected_event_regimes`）**：
    人工年表覆盖历史、但不可能预先写尽未来。每日 RSS 抓取的真实事件经**高门槛规则核验**（标题须命中
    地缘/供给/需求高影响词、情感与强度达阈值，`events` 表只增不删、按 日期+标题 去重、全部带来源 URL），
    把库内累积 + 当日的事件从**报道日起**按主题做指数衰减展开（地缘 20 / 供给 15 / 需求 20 个交易日，
    非交易日归到下一交易日、持续报道取 max 刷新、**报道日之前一律为 0，杜绝未来泄漏**），再与人工年表
    **逐主题取 max 合并**入模。对华供油链路（霍尔木兹/伊朗/油轮/波斯湾/Abqaiq 等关键词）单独识别为
    `hist_china_supply_regime`；俄乌、红海等不含对华供油关键词的冲突只计地缘、不误标对华。低强度日常
    库存/行情噪声（强度 <0.5）被门槛过滤。这样新爆发的重大事件（如新一轮油轮战、设施遇袭）无需改代码，
    次日即自动进入特征与权重学习；人工年表仍作为权威地缘窗口（`major_event_regime`、条件敏感度）的依据。
  - **长期平均权重 ≠ 当前事件期敏感度**：因素热力图是近 500 交易日全窗平均，会把当前冲突期的
    高敏感度稀释；报告另用真实价格、**只在纯地缘类事件窗口**独立统计"重大事件期条件敏感度"
    （事件期/非事件期波动比、对滞后 Brent-WTI 地缘溢价的传导 β 及其抬升 `beta_lift`、事件期
    累计涨跌、**相对布伦特的累计超额 `event_excess_vs_brent_pct`**），并配对比表/图——例如
    上海 SC 全窗平均地缘权重不突出，但在霍尔木兹/伊朗窗口相对布伦特累计超额与 β 抬升居三大
    原油之首，两口径并列、不互相冒充。
...
- **每日重训 + 模型持久化热启动（持续学习、重启不从零）**：默认抓取并**用满近 10 年
  ...
  **工件以 joblib 落盘到 `models/artifacts/`，`manifest.json` 记录版本链（parent_version）、
  ...
  - **运行性能与断点续跑（适配 Streamlit Cloud 同步时限）**：方向门控 edge 是回看约 5 年、
    数百非重叠原点的**慢变统计**，每日全量重算代价大却几乎不变，因此 `dir_gate_refresh_days`
    天内自动**复用上期门控证据、只重训最终方向分类器与 direct 回归**（单品种由冷启动约 36s
    降到约 11s），到期/冷启动/特征 schema 变化才全量重算；门控原点按 `dir_gate_max_origins_per_h`
    以 h 整数倍抽稀（标签区间仍互不重叠、不破坏二项检验独立性）。ARIMA 基准默认关闭（实测在
    交易日 index 上长期收敛失败、零贡献；已修复其索引问题，确需可 `run_arima=True` 开启）。
    管线**每训练完一个品种立即落盘**，万一首次冷启动被平台超时中断，再次点击会对已完成品种
    走热启动秒级跳过、只补未完成品种（断点续跑，不必从头）。
  - **回测结果跨期缓存（消除每日管线最大尾部耗时）**：滚动样本外回测是对历史的慢变评估，
    生产配置下逐原点完整拟合曾使每日管线在回测阶段超过 Streamlit Cloud 同步时限。现 ① 回测
    逐原点改用评估专用小校准规模（`backtest_calib_origins/iter`，不影响线上模型质量），原点
    30×6 仍覆盖约 9 个月，单次回测由约 10 分钟降到约 95 秒；② 结果按主锚数据末日写入
    `data/backtest_cache_<anchor>.json`，`backtest_refresh_days`(7) 天内直接复用并在报告标注
    截止日，到期/数据推进才重算（仅缓存 available=True 的结果，不可用绝不缓存）。中期 VAR 的
    交易日索引重置为整数索引，消除 statsmodels 频率告警。实测五品种真实库：冷启动约 8.7 分钟
    （仅仓库无工件时一次），带工件的日常运行约 4 分钟，均在平台时限内。
...
- **近端正向确认（regime 自适应熔断）**：长历史 walk-forward 只能证明方向信号"过去若干年
  总体有效"，无法保证当前行情仍有效——地缘冲击/V 型反转会让信号从有效转为失效甚至稳定反向。
  因此在长历史 edge 之外，门控还要求**最近约 1 年（`edge_recent_span`）已实现的样本外表态
  近端确实有效**：ML 概率通道须"按方向平均赚钱"且"命中过半"，趋势通道只要求平均收益为正
  （趋势跟随允许低胜率高盈亏比）；不满足即对该通道熔断、退回中性，直到近端 edge 恢复，近端
  表态样本不足时不做否决（避免小样本误杀）。这杜绝了"长历史曾有效、近期已与抛硬币无异甚至
  稳定逆势"时仍高比例硬表态。
- **回测门控逐原点滚动 + 训练/实际窗口严格对齐**：滚动回测的方向门控不再固定在回测起点前
  判一次，而是用一个**密集、覆盖回测窗口并前铺 1 年**的样本外探针，在每个原点 t 只引用
  `i+h≤t`（当时已实现）的点做近端正向确认，校准映射沿用起点前的长历史校准（稳定、不被一二十
  个近期点小样本在线自适应而绕过熔断），与生产"约每周刷新门控"同口径、全程无未来泄漏。同时
  修正了回测训练窗止于 t-1、实际收益却从 t 起算的 **1 个交易日错位（off-by-one）**：现训练
  含原点 t、喂第 t 行特征、实际收益从 `price[t]→price[t+h]` 起算，与生产 `predict` 完全一致。
...
| `system.network.proxy_url` | 固定代理（留空直连；运行期 CLI/环境变量/`data/proxy.json` 优先级更高） | 空 |
| `system.network.proxy_pause_enabled` / `proxy_pause_seconds` | 不可达暂停开关（auto/on/off）与倒计时秒数 | auto / 60 |
