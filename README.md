...
- **五品种统一到同一真实时刻（不止上海）**：在分时覆盖窗口内，**WTI、Brent、美燃油、伦敦柴油、上海原油
  五个品种在每个交易日都取北京 02:30 这同一瞬间的盘中成交价**（此时欧美电子盘仍在交易，用户已确认按盘中价、
  不要求开/收盘价）。此前只把上海切到 02:30、海外四品种仍用各自更晚的美盘日收盘，导致"上海日 K −7% 而外盘
  同刻仅 −1%"的虚假背离；修正后五品种变化率/波动/敏感度才真正可比（实证 2026-09-18 02:30 同刻：
  WTI 99.60、Brent 103.17、HO 5.04、Gasoil 1485.25、上海 734.8 元）。原始日 K（15:00/美盘收盘）仍落库
  保留、不改写，仅在分析与报告层用同刻面板覆盖。上海 02:30/15:00 面板同样按 4.4 的换月复权因子校正
  （`roll_adjust_panel_sc`），使价格序列、跨市场外生特征、美元升贴水三处口径一致。
- **双口径标注、不混用**：上海原油在分时覆盖日**自身收盘价、跨市场 Brent/WTI、美元换算与对布升贴水统一用
  02:30 同时刻价**，15:00 日盘截面仅作对照；更早日期维持 4.4/交易时段 as-of 日频口径。**免费分时源历史有限**
  （新浪 15 分钟 K 仅回溯约百余交易日、CNBC `1H` 约 3 个多月，实测同刻面板自 2026-06-03 起），十年历史
  回测段无法逐时刻对齐，只能用日 K + 特征层 as-of；这是数据可得性的硬边界，报告与代码均如实标注、不宣称
  十年全程同刻。面板随每日增量积累，覆盖窗口会越来越长。
