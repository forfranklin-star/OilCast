    ...
    def fit(self, prices: pd.DataFrame, macro: pd.DataFrame, target: str = "wti",
        ...
        if df.shape[1] < 2:
            raise InsufficientData(
                f"VAR 除油价自身外无其他真实覆盖充分的宏观内生变量（纳入列 {self.used_cols}），拒绝训练")
        ...
        # 交易日 DatetimeIndex 无固定频率会让 statsmodels 反复发 "date index has no
        # associated frequency" 告警；VAR 只用等间距行序（预测日期由 future_dates 显式
        # 传入），重置为整数 RangeIndex 即可，结果不变、日志干净。
        self.data = df.clip(lower=lo, upper=hi, axis=1).reset_index(drop=True)
