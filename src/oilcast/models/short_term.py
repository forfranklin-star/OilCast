...
class ShortTermForecaster:
    def __init__(self, horizon: int = 10, window: int = 180,
        ...
        # 带原点位置的样本外原始方向序列（供回测逐原点近端熔断切窗），随工件持久化。
        self.gate_pa_raw: Dict[int, list] = {}
        self.gate_trend_raw: Dict[int, list] = {}
    ...
    def _direction_walkforward(self, X: pd.DataFrame, log_p: pd.Series) -> None:
        ...
        # 带原点位置的样本外原始序列（rp 未经全样本 isotonic，严格 OOS），供回测按
        # "原点 t 时仅 i+h<=t 已实现"切近端窗口做逐原点熔断，无未来泄漏。
        self.gate_pa_raw = {h: [] for h in hs}
        self.gate_trend_raw = {h: [] for h in hs}
        ...
        for h in hs:
            ...
            for t in origins:
                ...
                if (np.isfinite(m21) and np.isfinite(cut) and np.isfinite(fwd_ret)
                    ...
                    self.gate_trend_raw[h].append((t, float(np.sign(m21)), fwd_ret))
                lo = max(0, t - win + 1)
                tr_X = X.iloc[lo:t + 1]
                ...
                try:
                    ...
                    if np.isfinite(p_up) and np.isfinite(fwd_ret):
                        ...
                        self.gate_pa_raw[h].append((t, p_up, actual_dir, fwd_ret))
                except Exception:
                    continue
    ...
    @staticmethod
    def _recent_failure(eng_sign, fwd, min_eng: int, max_p: float,
                        ad=None, require_hit: bool = True) -> dict:
        """近端正向确认门控（regime 自适应）。

        长历史 walk-forward 只能证明方向信号"过去若干年总体有效"，无法保证当前行情仍有效；
        地缘冲击/V 型反转期，方向信号会从有效转为失效甚至反向。本方法只看【最近一段已实现
        的样本外原点】（实际收益在当前预测日之前已发生，无未来函数），是当前是否允许表态的
        【正向确认】开关，而非仅在显著坏掉时才熔断：

        - ML 概率通道 (require_hit=True)：近端高置信表态必须【平均按方向赚钱】且【命中过半】，
          任一不满足即退回中性——近端与抛硬币无异或亏钱时，不允许靠"长历史曾经有效"继续表态；
        - 趋势通道 (require_hit=False)：趋势跟随天然胜率可低、靠盈亏比盈利，故只要求近端按
          信号持有的【平均收益为正】，不以胜率过半为必要条件。
        近端表态样本少于 min_eng 时证据不足，不做近端否决（回退长历史结论，避免小样本误杀）。
        判定一律用每原点当时的样本外原始概率/信号，不用全样本校准器，杜绝校准泄漏。

        eng_sign: 每原点表态方向 (+1 涨 / -1 跌 / 0 未表态)；fwd: 同期实际对数收益；
        ad: 实际涨跌 (1/0)，给定时按方向命中统计，否则用收益符号（趋势通道）。"""
        out = {"recent_total": int(len(eng_sign)), "recent_engaged_n": 0,
               "recent_hit": None, "recent_mean_ret": None, "recent_p": None,
               "recent_ok": True, "recent_reason": None}
        sgn_all = np.asarray(eng_sign, dtype=float)
        m = sgn_all != 0
        en = int(m.sum())
        out["recent_engaged_n"] = en
        if en < min_eng:
            out["recent_reason"] = f"近端表态样本不足({en}<{min_eng})，不做近端否决"
            return out
        sgn = sgn_all[m]
        f = np.asarray(fwd, dtype=float)[m]
        strat = sgn * f
        if ad is not None:
            a = np.asarray(ad)[m]
            hit = float((((sgn == 1) & (a == 1)) | ((sgn == -1) & (a == 0))).mean())
        else:
            hit = float((np.sign(f) == sgn).mean())
        mean = float(strat.mean())
        tp = float(ttest_1samp(strat, 0.0).pvalue) if en >= 3 else 1.0
        bp = float(binomtest(int(round(hit * en)), en, 0.5).pvalue)
        out.update(recent_hit=round(hit, 3), recent_mean_ret=round(mean * 100, 3),
                   recent_p=round(tp, 3), recent_binom_p=round(bp, 3))
        fail_econ = bool(mean < 0)          # 近端按表态方向不赚钱（经济意义否决）
        fail_hit = bool(require_hit and hit < 0.5)  # ML 通道还要求命中过半
        if fail_econ or fail_hit:
            out["recent_ok"] = False
            if fail_econ:
                out["recent_reason"] = "近端按表态方向平均收益为负（已不赚钱）"
            else:
                out["recent_reason"] = "近端方向命中不足50%（无方向edge）"
        return out
    ...
    def _fit_direction_gate(self, hs, dir_pa, mcfg, trend_pa=None,
        ...
        # 近端失效熔断：长历史 edge 成立，仅当【最近约 recent_span 个交易日】该通道样本外
        # 表态仍未稳定亏钱/显著反向时才允许当前明确表态，否则退回中性（regime 自适应）。
        recent_span = int(mcfg.get("edge_recent_span", 250))
        recent_min_eng = int(mcfg.get("edge_recent_min_eng", 6))
        recent_max_p = float(mcfg.get("edge_recent_max_p", 0.30))
        trend_pa = trend_pa or {}
        self.dir_calib, self.dir_edge, self.trend_edge = {}, {}, {}
        for h in hs:
            k = max(recent_min_eng, int(np.ceil(recent_span / h)))
            te = self._trend_gate(trend_pa.get(h, []), trend_cut,
                                  min_eng, min_payoff, max_ret_p)
            # 趋势通道近端熔断（trend_pa 元素=(mom符号, 顺势持有实际对数收益)，均为表态点）
            tarr = trend_pa.get(h, [])
            t_recent = tarr[-k:]
            t_rec = self._recent_failure([a[0] for a in t_recent],
                                         [a[1] for a in t_recent],
                                         recent_min_eng, recent_max_p, ad=None,
                                         require_hit=False)
            te_long = bool(te["has_edge"])
            te["has_edge"] = bool(te_long and t_rec["recent_ok"])
            te.update(recent_ok=t_rec["recent_ok"], recent_engaged_n=t_rec["recent_engaged_n"],
                      recent_hit=t_rec["recent_hit"], recent_mean_ret=t_rec["recent_mean_ret"],
                      recent_p=t_rec["recent_p"], recent_reason=t_rec["recent_reason"])
            ...
            trend_fields = {"trend_n": te["n"], "trend_hit": te["hit"],
                            ...
                            "trend_p": te["p"], "trend_recent_ok": t_rec["recent_ok"],
                            "trend_recent_hit": t_rec["recent_hit"],
                            "trend_recent_mean_ret": t_rec["recent_mean_ret"]}

            def _empty(reason):
                return {**base, "has_edge": te["has_edge"],
                        ...
                        "engaged_ret_p": None, "ml_recent_ok": None,
                        "ml_recent_hit": None, "ml_recent_mean_ret": None,
                        "ml_recent_reason": None, **trend_fields, "reason": reason}

            if len(arr) < min_n:
                self.dir_calib[h] = None
                self.dir_edge[h] = _empty("样本外原点不足")
                continue
            ...
            ml_long = bool(hit_edge or econ_edge)
            # ML 通道近端熔断：用最近 k 个【样本外原始概率】（不经全样本校准，无泄漏）按
            # 同一置信带判表态，若近期按表态持有显著亏钱/显著反向则关闭 ML 通道。
            r_rp, r_ad, r_fr = rp[-k:], ad[-k:], fr[-k:]
            r_sign = np.where(r_rp >= thr, 1, np.where(r_rp <= 1 - thr, -1, 0))
            m_rec = self._recent_failure(r_sign, r_fr, recent_min_eng,
                                         recent_max_p, ad=r_ad)
            ml_ok = bool(m_rec["recent_ok"])
            ml_edge_now = bool(ml_long and ml_ok)
            # 最终 edge：ML 与趋势两通道各自通过"长历史有效 + 近端未失效"才放行
            has_edge = ml_edge_now or te["has_edge"]
            if ml_edge_now:
                edge_basis = "胜率" if hit_edge else "趋势期望"
            elif te["has_edge"]:
                edge_basis = "趋势期望"
            else:
                edge_basis = None
            gate_note = None
            if ml_long and not ml_ok:
                gate_note = f"ML方向edge长历史成立但近端失效（{m_rec['recent_reason']}），近端退回中性"
            elif te_long and not t_rec["recent_ok"]:
                gate_note = f"趋势edge长历史成立但近端失效（{t_rec['recent_reason']}），近端退回中性"
            ...
            self.dir_edge[h] = {
                **base, "has_edge": has_edge, "edge_basis": edge_basis,
                ...
                "engaged_ret_p": round(ret_p, 3),
                "ml_long_edge": ml_long, "ml_recent_ok": ml_ok,
                "ml_recent_engaged_n": m_rec["recent_engaged_n"],
                "ml_recent_hit": m_rec["recent_hit"],
                "ml_recent_mean_ret": m_rec["recent_mean_ret"],
                "ml_recent_p": m_rec["recent_p"],
                "ml_recent_reason": m_rec["recent_reason"],
                "gate_note": gate_note, **trend_fields}
