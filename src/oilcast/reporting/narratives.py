...
def conditional_sensitivity_narrative(cs: dict, names: dict) -> str:
    ...
    for t in crude:
        ...
        if d.get("vol_ratio") and d.get("event_cumret_pct") is not None:
            segs.append(f"{names[t]}事件期波动放大 {d['vol_ratio']} 倍、累计 {d['event_cumret_pct']:+.1f}%，"
                        f"地缘溢价传导 β 由非事件期 {d.get('geo_beta_normal')} 变为 {d.get('geo_beta_event')}"
                        f"（抬升 {d.get('beta_lift')}）")
    extra = ""
    if lead_ret == "shanghai_crude" or lead_lift == "shanghai_crude":
        extra = ("。上海原油（INE SC）可交割标的为中东含硫原油、中国是伊朗及中东原油主要买家，"
                 "在本轮事件期的累计涨幅与地缘敏感度抬升均居三大原油之首，高于 WTI（锚定美国本土页岩、"
                 "自给度高、对中东断供最钝），这解释了为何其全窗平均地缘权重看似不高、当前行情却由地缘主导")
