...
def sensitivity_figure(report: dict) -> Optional[go.Figure]:
    ...
    fig.update_layout(title="分品种因素敏感度对比（各油种独立学到的权重，%）",
                      height=430, margin=dict(l=150, r=20, t=56, b=30),
...
def conditional_sensitivity_figure(report: dict) -> Optional[go.Figure]:
    ...
    items = [(t, d) for t, d in cs.items() if isinstance(d, dict) and d.get("available")
             and d.get("beta_lift") is not None]
    if len(items) < 2:
        return None
    names = [report["names"][t] for t, _ in items]
    lift = [round(float(d["beta_lift"]), 3) for _, d in items]
    cum = [round(float(d["event_cumret_pct"]), 1) if d.get("event_cumret_pct") is not None else None
           for _, d in items]
    fig = go.Figure()
    # 左轴：事件期累计涨跌%（柱）；右轴：地缘β抬升（折线+点）。两个量纲分轴、不重叠，
    # 避免双轴分组柱因尺度悬殊造成"满高柱盖住另一根"的视觉误导。
    fig.add_trace(go.Bar(name="事件期累计涨跌%", x=names, y=cum,
                         marker_color="#b03434", text=cum, textposition="outside", yaxis="y"))
    fig.add_trace(go.Scatter(name="事件期地缘β抬升 beta_lift", x=names, y=lift,
    ...
    fig.update_layout(
        ...
        height=380, margin=dict(l=50, r=50, t=56, b=30),
        plot_bgcolor="white", legend=dict(orientation="h", y=-0.16), font=dict(size=12),
        bargap=0.55)
    fig.update_yaxes(title="累计涨跌%", showgrid=True, gridcolor="#eef1f5")
    fig.update_layout(yaxis2=dict(title="β抬升", overlaying="y", side="right", showgrid=False, range=[0, max([x for x in lift if x is not None] + [0.1]) * 1.35]))
    return fig
...
<div class="wrap">
 ...
 <div class="card" style="margin-top:16px">
   ...
   <div style="color:#666;font-size:12px;margin-bottom:8px">仅用真实价格统计：事件期/非事件期年化波动比、日收益对滞后地缘溢价（布伦特-WTI 价差 5 日变化）的传导 β 及其抬升、事件期累计涨跌。上海 INE SC 可交割中东含硫油、中国是伊朗原油主要买家，故在对华供油受扰窗口 β 抬升与累计涨幅往往居前。样本不足留空，不估算。</div>
   <div id="fig_cond"></div>
   <table style="width:100%;border-collapse:collapse;font-size:13px">
     <thead><tr style="background:#f5f7fa;text-align:right">
       ...
       <th style="padding:6px">事件期累计涨跌</th><th style="padding:6px">事件交易日占比</th>
     </tr></thead>
     <tbody>
