...
FACTOR_GROUPS: Dict[str, list] = {
    # hist_supply_regime：覆盖十年、可审计的 OPEC+/SPR 产量政策与实物中断（寒潮/运河）状态锚；
    # ev_supply_5d 是近 30 天新闻文本的时变边际。
    "supply_disruption": ["ev_supply_5d", "hist_supply_regime"],
    # 地缘因素：日频文本（GPR/ev_geo/china_supply，RSS 仅回溯近 30 天）+ 十年可审计历史
    # 事件状态锚。hist_geo_regime=战争/制裁/袭船/设施遇袭；hist_china_supply_regime 专门标记
    # 直接扰动对华中东供油链路（伊朗制裁、海湾油轮战、阿美遇袭、霍尔木兹封锁）的事件，使上海
    # INE 对"对华供油"的高敏感能在 2018/2019/2025/2026 多起事件上被模型学到，而非只看最近几天。
    "geopolitical_risk": ["gpr_level_z", "gpr_chg_5d", "ev_geo_5d", "geo_premium_chg5",
                          "china_supply_risk_5d", "hist_geo_regime",
                          "hist_china_supply_regime"],
    ...
    # hist_demand_regime：十年需求侧冲击状态锚（如新冠需求崩塌）。
    "demand_outlook": ["demand_chg_5d", "ev_demand_5d", "hist_demand_regime"],
    "institutional_view": ["inst_bias", "ev_view_5d"],
    "_technical_": [
        "ret_1d", "ret_5d", "ma_gap", "vol_20",
    ],
}
# 全部重大事件（不分主题）的总风险环境序列：入模作为上下文，但不重复计入某一展示因素
# （分主题归因由 geopolitical/supply/demand 各自的 hist_* 承担）。
FACTOR_GROUPS["_eventcontext_"] = ["major_event_regime"]
...
def _event_regime_subset(index: pd.DatetimeIndex, events: list, predicate) -> pd.Series:
    """按 predicate(event) 筛选年表事件，复用 major_event_regime 的窗口+phases+取 max 逻辑，
    生成一条 0~强度 的日频状态序列；事件重叠取强度最大者。"""
    ...
    for e in events:
        ...
        try:
            st = pd.Timestamp(e["start"])
            en = pd.Timestamp(e["end"]) if e.get("end") else index.max()
            intensity = float(e.get("intensity", 1.0))
        except Exception:
            continue
        mask = (index >= st) & (index <= en)
        s.loc[mask] = np.maximum(s.loc[mask], intensity)
        for ph in (e.get("phases") or []):
            try:
                pst = pd.Timestamp(ph["start"])
                pen = pd.Timestamp(ph["end"]) if ph.get("end") else index.max()
                pv = float(ph.get("intensity", intensity))
            except Exception:
                continue
            pmask = (index >= pst) & (index <= pen) & mask
            s.loc[pmask] = np.maximum(s.loc[pmask], pv)
    return s


# 事件 category → 分主题历史 regime
_HIST_REGIME_SPECS = {
    "hist_geo_regime": lambda e: e.get("category") == "geopolitical",
    "hist_supply_regime": lambda e: e.get("category") in ("supply_policy", "supply_outage"),
    "hist_demand_regime": lambda e: e.get("category") == "demand_shock",
...
def conditional_geo_sensitivity(prices: pd.DataFrame, targets: list,
                                ...
                                usdcny: Optional[pd.Series] = None) -> Dict[str, dict]:
    """事件期【条件敏感度】归因——回答"当前地缘冲突中各品种到底有多敏感"。

    因素权重（模型特征重要性）是近 500 交易日的全窗平均，会把当前事件期的高敏感度
    稀释；本函数只用真实价格做可审计统计，独立于树模型。主指标是不依赖任何价差代理、
    最稳健的两项：
      * event_cumret_pct / event_excess_vs_brent_pct：重大事件窗口内的累计涨跌、以及
        相对布伦特的累计超额（直接回答"冲突中谁涨得多"，上海 INE SC 因可交割中东含硫油、
        中国是伊朗原油主要买家，在对华供油受扰窗口往往录得正超额）；
      * vol_ratio ：事件期年化波动 / 非事件期年化波动（>1 表示冲突中波动放大）。
    参考指标 geo_beta：日收益对外生的 Brent-WTI 地缘溢价【滞后 5 日变化】的 OLS 斜率
      （Brent-WTI 不含任一被解释品种自身价格，对上海 SC 尤其干净；滞后避免机械恒等），
      事件期与非事件期之差 beta_lift 为冲突带来的传导抬升。价差代理对设定敏感，仅作参考。
    样本不足（<20）的统计项返回 None，绝不造数。usdcny 入参为接口兼容保留、当前不使用。
    """
    prices = prices.sort_index()
    idx = prices.index
    # 事件期只取【地缘类】事件（战争/制裁/袭船/设施遇袭，含全部对华供油链路事件），
    # 不含 OPEC 产量政策、寒潮/运河等实物中断与疫情需求冲击，避免污染"地缘敏感度"归因。
    regime = historical_event_regimes(idx)["hist_geo_regime"]
    out: Dict[str, dict] = {}
    # 外生地缘溢价横轴（滞后一期的 5 日变化）：Brent-WTI 不含 SC 自身价格，对所有品种一致可比
    bw = None
    ...
    rb_sum = None
    if "brent" in prices:
        rb = np.log(prices["brent"].ffill(limit=3)).diff().reindex(win_idx)
        rb_ev = rb[ev_mask].dropna()
        if len(rb_ev) >= 20:
            rb_sum = float(rb_ev.sum())
    ...
    for t in targets:
        ...
        x = bw.reindex(win_idx) if bw is not None else pd.Series(np.nan, index=win_idx)
        ...
        # 事件期相对布伦特累计超额（对数域相减后还原），基准缺失则留 None
        excess = (float(np.expm1(r_ev.sum() - rb_sum) * 100)
                  if (len(r_ev) >= 20 and rb_sum is not None and t != "brent") else
                  (0.0 if (len(r_ev) >= 20 and t == "brent") else None))
        out[t] = {
            "available": True,
            "proxy": "Brent-WTI价差5日变化(滞后)",
            ...
            "event_excess_vs_brent_pct": round(excess, 1) if excess is not None else None,
...
def build_features(prices: pd.DataFrame, macro: pd.DataFrame,
    ...
    # 统一用布伦特-WTI 对数价差 5 日变化。它是【外生于上海 SC】的全球海运中质油地缘锚
    # （Brent 锚中东/海运、WTI 锚美国本土页岩，冲突升级走阔），对 SC 是领先信号、不含 SC
    # 自身价格、无均值回复污染。上海 INE SC"对华中东供油链路更敏感"的特异性，不用含自身
    # 价格的 SC-Brent 升贴水在此重复表达（该量已在跨市场组 spread_brent_chg5 中），而由
    # major_event_regime（全历史事件状态）、china_supply_risk_5d（霍尔木兹/油轮文本强度）
    # 与报告"事件期相对布伦特异超额收益"共同刻画。
    if "brent" in prices and "wti" in prices:
