...
def run(as_of: Optional[datetime] = None, require_prices: bool = False,
    ...
    # 8) 回测（仅主锚）。原点个数/间隔从配置读取，保证样本量统计可信
    _mcfg = get_config()["model"]
    if anchor:
        bt = backtest_short(
            feats[anchor], prices[anchor], horizon=h_short,
            n_origins=int(_mcfg.get("backtest_origins", 42)),
            gap=int(_mcfg.get("backtest_gap", 3)))
    else:
    ...
    cond_sens = conditional_geo_sensitivity(prices, list(TARGETS.keys()),
                                            recent_window=int(mcfg.get("train_window",
                                                                       mcfg.get("rolling_window", 500))))
