...
def run(as_of: Optional[datetime] = None, require_prices: bool = False,
    ...
    cfg = get_config()
    ensure_dirs()
    net.export_env_proxy()        # 启动即把已保存/环境代理注入进程，所有 HTTP 客户端统一生效
...
def main() -> None:
    ...
    parser.add_argument("--proxy", type=str, default=None, metavar="URL",
                        help="设置并持久化代理（如 http://127.0.0.1:7890、socks5://127.0.0.1:1080）；"
                             "传 off/none 清除代理改用直连")
    parser.add_argument("--no-proxy", action="store_true", help="清除代理、改用直连并持久化")
    parser.add_argument("--proxy-test", action="store_true",
                        help="测试当前代理（或 --proxy 指定者）能否访问探测地址后退出")
    args = parser.parse_args()

    # 代理设置优先处理（最先注入环境，后续采集/模型全部走该通道）
    if args.no_proxy:
        net.set_proxy("")
        print("已清除代理，当前：直连")
    if args.proxy:
        u = "" if args.proxy.lower() in ("off", "none", "direct", "直连", "") else args.proxy
        print("已设置代理：", net.set_proxy(u) or "直连")
